 ...
 *  - Guest performs write-only uploads from guest memory to the host.
 *
 ...
 * In C, declare them as extern; your WASM toolchain should link these imports.
 */
 ...
 * Export mechanics vary by toolchain; you typically need:
 *   - no name mangling
 *   - default visibility / export attribute
 *
...
/* Convert a guest linear-memory offset into a host-visible pointer in C.
 * In a WASM guest, linear memory starts at address 0, so this cast is typical.
 */
